...
export const PanelControl: React.FC<PanelControlProps> = ({
  ...
  const [subTab, setSubTab] = useState<"historial" | "dashboard" | "documentos">("historial");
  ...
  const pdfFiles = React.useMemo(() => {
    const list: { id: number; title: string; filename: string; type: "manifiesto" | "certificado"; row: RetiroExterno; date: string }[] = [];
    retiros.forEach((r) => {
      if (r.pdfCargado) {
        list.push({
          id: r.id,
          ...
          type: "manifiesto",
          row: r,
          date: r.fechaManifiesto || r.fecha
        });
      }
      if (r.pdfCertificadoCargado) {
        list.push({
          ...
          type: "certificado",
          row: r,
          date: r.fechaTratamiento || r.fecha
        });
      }
    });
    return list;
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white border border-slate-200 p-3 rounded-2xl shadow-sm">
          ...
          <div className="flex gap-2 p-1 bg-slate-100 rounded-xl">
            <button
              onClick={() => { 
                setTab("interno"); 
                if (subTab === "documentos") setSubTab("historial");
                setPageToOne(); 
              }}
          ...
          {/* Sub Tabs: LISTING VS CHARTS VS DOCUMENTS */}
            ...
            <div className="space-y-6">
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2.5 mb-2">
                  ...
                  <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider m-0">Repositorio de Documentos Digitales</h3>
                </div>
                <p className="text-xs text-slate-500 leading-relaxed max-w-3xl m-0">
                  Acceso directo a todos los archivos PDF cargados durante la gestión de retiro externo. En esta sección podrá visualizar e imprimir los manifiestos electrónicos de transporte y los certificados de disposición final emitidos por los operadores autorizados.
                </p>
              </div>
              ...
              {pdfFiles.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
                  <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-4 border border-slate-200">
                    <FileMinus className="w-7 h-7 text-slate-400" />
                  </div>
                  <h4 className="font-extrabold text-sm text-slate-700 uppercase tracking-wider mb-1">Sin archivos cargados</h4>
                  <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed m-0">
                    Aún no se han cargado documentos en los despachos externos de residuos. Al registrar un retiro externo, recuerde arrastrar o seleccionar los archivos PDF correspondientes.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pdfFiles.map((file) => {
                    const color = CAT_COLOR[file.row.categoria] || "#64748b";
                    return (
                      <div key={file.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                        <div className="flex gap-4">
                          <div className="w-11 h-11 rounded-xl bg-red-50 flex items-center justify-center border border-red-100 flex-shrink-0">
                            <FileText className="w-5 h-5 text-red-600" style={{ color: "#dc2626" }} />
                          </div>
                          <div className="space-y-1 min-w-0">
                            <h4 className="font-extrabold text-xs text-slate-800 leading-snug truncate" title={file.title}>
                              {file.title}
                            </h4>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                              Categoría: <span className="font-bold whitespace-nowrap px-1.5 py-0.5 rounded text-[9px] border inline-block" style={{ backgroundColor: `${color}10`, color, borderColor: `${color}30` }}>{file.row.categoria}</span>
                            </p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                              Corriente: <span className="font-semibold text-slate-600">{file.row.corriente}</span>
                            </p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                              Fecha Doc: <span className="font-semibold text-slate-600">{file.date}</span>
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="border-t border-slate-100 pt-3.5 mt-4 flex items-center justify-between">
                          <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                            file.type === "manifiesto" 
                              ? "bg-sky-50 text-sky-700 border border-sky-100" 
                              : "bg-emerald-50 text-emerald-700 border border-emerald-100"
                          }`}>
                            {file.type === "manifiesto" ? "Manifiesto" : "Certificado"}
                          </span>
                          <button
                            onClick={() => {
                            ...
                            className="text-xs font-bold text-sky-600 hover:text-sky-850 hover:underline inline-flex items-center gap-1 cursor-pointer transition-colors"
                          >
                            Visualizar PDF &rarr;
                          </button>
                        </div>
                      ...
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-xl border text-xs font-semibold flex items-center gap-2.5 transition-all text-xs ${
                          isSelected
                            ...
                            : "bg-white border-slate-200 hover:border-slate-350 text-slate-700 font-medium"
                        }`}
                      >
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-slate-200">
              ...
              <div className="bg-slate-900 px-6 py-4 flex items-center justify-between text-white flex-shrink-0">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-white p-1.5 hover:bg-slate-800 rounded-lg transition-colors cursor-pointer animate-none"
                ...
                <div className="bg-white p-8 rounded-xl shadow-2xl border border-slate-350 w-full max-w-2xl min-h-[750px] flex flex-col justify-between text-slate-800 relative font-sans">
                    ...
                    <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-200 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-y-1.5 gap-x-4">
                        ...
                        <div>
                          ...
                          <p className="font-extrabold text-slate-800 text-xs">{selectedPdfRow.cantEst || selectedPdfRow.cantidad} {selectedPdfRow.unidad}</p>
                        </div>
                        <div>
                    ...
                    <div className="bg-sky-50/20 p-4 rounded-xl border border-sky-100/60 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-y-1.5 gap-x-4">
                        ...
                        <div>
                          ...
                          <p className="font-mono font-bold text-slate-800 text-xs">{selectedPdfRow.fechaRetiro || selectedPdfRow.fecha}</p>
                        </div>
                        <div>
                    ...
                    <div className="bg-emerald-50/20 p-4 rounded-xl border border-emerald-100/60 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-y-1.5 gap-x-4">
                        ...
                        <div>
                          ...
                          <p className="font-mono font-bold text-slate-800 uppercase tracking-widest bg-white inline-block px-1.5 py-0.5 rounded border border-slate-300 text-xs">{selectedPdfRow.patente || "N/A"}</p>
                        </div>
                        <div>
                          <span className="text-slate-400 font-semibold uppercase text-xxs block mb-0.5">Fecha de tratamiento</span>
                          <p className="font-mono font-bold text-slate-700 text-xs">{selectedPdfRow.fechaTratamiento || selectedPdfRow.fecha}</p>
                        </div>
                        <div>
                  ...
                  <div className="border-t border-slate-200 pt-5 mt-auto flex justify-between items-end">
                    <div className="text-left">
                      <div className="text-[7px] font-mono text-slate-400 tracking-tight leading-normal uppercase">
                        CÓDIGO SGI DIGITAL DE AUTORIZACIÓN<br />
                        |||| | | |||| || ||||| ||| ||| | ||||| | ||
                        <span className="block mt-0.5 text-[8px] tracking-[0.22em] font-bold text-slate-600 uppercase">SGI-TRAZA-{selectedPdfRow.id}</span>
                      </div>
                    </div>
              ...
              <div className="bg-slate-900 border-t border-slate-800 px-6 py-4 flex gap-3 justify-end flex-shrink-0">
                <button
                  ...
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-slate-600 rounded-xl text-xs font-bold font-mono text-white flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-95 animate-none"
                >
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-bold font-mono text-white flex items-center justify-center gap-1.5 cursor-pointer transition-all active:scale-95 shadow-sm animate-none"
                >
